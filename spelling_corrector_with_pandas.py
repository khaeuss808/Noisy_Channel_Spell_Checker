# observation x is the mispelled word
# find the correct word w

# noisy channel is maximizing the liklihood of P(x|w) and P(w)


# train the unigram model

# implement the weighted Levenshstein distance model

# combine model to create the noisy channel formula
#
...
def spelling_corrector(bad_word):
    ...
    bigram = pd.read_csv("bigrams.csv")
...
def deletion_prob(errorcode, deletions_data, bigrams_data):
    ...
    denom_count = bigrams_data.loc[bigrams_data["bigram"] == errorcode, "count"].values[
        0
    ]
    # print(denom_count)
...
def substitution_prob(errorcode, substitutions_data, unigrams_data):
    ...
    xi = errorcode[0]
    wi = errorcode[1]
    denom_count = unigrams_data.loc[unigrams_data["unigram"] == wi, "count"].values[0]

    try:  # doing this instead of just calling whats inside try because I tried to find rc in the substiutions.csv and saw it didnt exist there
        subs_count = substitutions_data.loc[
...
def addition_prob(errorcode, additions_data, unigrams_data):
    ...
    wiminus1 = errorcode[0]
    xi = errorcode[1]
    denom_count = unigrams_data.loc[
        unigrams_data["unigram"] == wiminus1, "count"
    ].values[0]

    try:  # doing this instead of just calling whats inside try because I tried to find rc in the substiutions.csv and saw it didnt exist there
        ins_count = additions_data.loc[
...
def get_all_words():
    ...
    for line in lines:
        ...
        # Store the word and frequency in the dictionary
